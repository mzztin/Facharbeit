<script lang="ts" context="module">
	...
	export const load: Load = async ({ page }) => {
		try {
			const res = await axios.get(`/users/${page.params.userId}`);
			const data = res.data;
			return { props: { exists: true, user: data as User } };
		} catch (e) { goto("/users/not_found"); }
	};
</script>
...
<style>
	main { gap: 12px; }
	.username { display: flex; flex-direction: row; }
